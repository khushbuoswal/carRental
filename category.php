      ...
      <div class="container">
        <div class="row">
          ...
          if ($json === false) {
            ...
            echo "<p>Unable to read cars.json file.</p>";
          } else {
            $cars = json_decode($json, true);
            if ($cars === null) {
              // Handle error if the JSON is invalid
              echo "<p>Invalid JSON format in cars.json file.</p>";
            } else {
              $filtered_cars = array_filter($cars, function ($car) use ($category) {
                return strtolower($car['type']) === strtolower($category);
              });
