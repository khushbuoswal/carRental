...
// Function to filter cars based on exact match
function filterCars($cars, $searchQuery) {
    return array_filter($cars, function ($car) use ($searchQuery) {
...
if (isset($_GET['query']) && !empty($_GET['query'])) {
    ...
    // Attempt to find exact matches first
    $searchResults = filterCars($cars, $searchQuery);

    // If no exact matches are found, find the best matches
    if (empty($searchResults)) {
        $searchResults = [];

        // Calculate similarity scores for each car
        foreach ($cars as $car) {
            $modelScore = similar_text($searchQuery, strtolower($car['model']), $modelPercent);
            $typeScore = similar_text($searchQuery, strtolower($car['type']), $typePercent);

            // Add the car and its highest similarity score to the results
            $searchResults[] = [
                'car' => $car,
                'score' => max($modelPercent, $typePercent)
            ];
        }
        ...
        // Keep only cars in the results
        $searchResults = array_column($searchResults, 'car');
    }
} else {
...
// Output the search results (for example, as JSON)
?>
...
<body style="background-color: rgb(243, 243, 228);">
    ...
    <div class="container" style="margin-top: 100px;">
        <h1 id="all-products" class="text-center">Search Results</h1>
